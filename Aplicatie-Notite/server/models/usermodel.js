...
class UserModel
{
     ...
     * @param {User} user 
     */
    async updateUser(userId, { username, password, photo }) {
        const db = this.#database;
        // const hashedPassword = password ? await encrypt.encryptPassword(password) : null;
        const hashedPassword = password ? `'${await encrypt.encryptPassword(password)}'` : 'password';
        const newPhotoPath = photo ? `'${photo.path}'` : 'photo';
        ...
        const updateQuery = `
            ...
            SET username = '${username}',
                ...
                photo = COALESCE(${newPhotoPath}, photo)
            WHERE id = ${userId}`;
      
        await db.runQuery(updateQuery);
        
        const userQuery = `SELECT id, username, photo FROM users WHERE id = ${userId}`;
        const updatedUser = await db.runQuery(userQuery);
      
        return updatedUser[0]; // Assuming query returns an array of results
    }
